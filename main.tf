...
locals {
  ...
  instances_by_region = {
    asia_southeast1 = { for k, v in var.instances : k => v if v.region == "asia-south1" }
    asia_east1      = { for k, v in var.instances : k => v if v.region == "asia-southeast1" }
    asia_east2      = { for k, v in var.instances : k => v if v.region == "us-central1" }
  }
}
...
resource "local_file" "ansible_inventory" {
  ...
  content = join("\n", flatten([
    ...
    # ClusterMaster group (add this block)
    "[ClusterMaster]",
    contains(keys(local.instances_metadata), "clustermanager") ?
    format("ClusterMaster ansible_host=%s instance_id=%s ansible_user=%s",
    ...
    "[IFs]",
    contains(keys(local.instances_metadata), "IF") ?
    format("IF ansible_host=%s instance_id=%s ansible_user=%s",
      local.instances_metadata["IF"].public_ip,
      local.instances_metadata["IF"].instance_id,
      var.ssh_user) : "",
...
resource "local_file" "ansible_group_vars" {
  ...
  content = templatefile("${path.module}/group_vars_template.yml", {
    cluster_master      = contains(keys(local.instances_metadata), "clustermanager") ? { "clustermanager" = local.instances_metadata["clustermanager"] } : {}
    ...
    Management_server   = contains(keys(local.instances_metadata), "Management_server") ? { "Management_server" = local.instances_metadata["Management_server"] } : {}
    IFs                 = contains(keys(local.instances_metadata), "IF") ? { "IF" = local.instances_metadata["IF"] } : {}
